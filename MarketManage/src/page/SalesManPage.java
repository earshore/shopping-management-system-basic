...
public final class SalesManPage extends ScannerChoice {
			...
			do {
				...
				if (choice.matches(regex)) {
					...
					case 1:
						...
						SalesMan salesManName = new SalesMan(salesMan.getSId(),
								sNewName, null);
						boolean boolsName = new SalesManDao().updateSalesMan(1,
								salesManName);
		...
		if (salesManList.size() <= 0) {
			...
			do {
				...
				if ("y".equals(choice) || "Y".equals(choice)) {
					// 进行刪除-数据库操作
					boolean boolDeleteSalesMan = new SalesManDao()
							.deleteSalesMan(sName);// 調用刪除功能
	...
	public static void querySalesManPage() {
		...
		ArrayList<SalesMan> salesManList = new SalesManDao()
				.querySalesMan(sName);
