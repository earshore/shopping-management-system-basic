...
public final class MainPage extends ScannerChoice {
	...
	public static void mainPage() {
		...
		do {
			...
			String regex = "[0-3]"; // 正则表达式，[]表示匹配中间的内容，0-3表示仅匹配0-3内的数字
			if (choice.matches(regex)) { // match函数匹配
	...
	public static void MaintenancePage() {
		...
		System.out.println("\n请输入选项,或者按 0 返回上一级菜单.");
		do {
			String choice = ScannerInfoString(); // 输入选择
			String regex = "[0-5]"; // []表示匹配中间的内容，0-5表示仅匹配0-5内的数字
			...
			if (choice.matches(regex)) {
				...
				case 1:
					...
					while (loginTimes != 0) {
						...
						ArrayList<SalesMan> salesManInfo = new SalesManDao()
								.checkstandLog(sName); // 以用户名从数据库中获取用户密码.
	...
	public static void shoppingSettlementPage(int salesManSid) {
		...
		do {
			...
			if ("0".equals(choNext)) {
				MainPage.checkstandLogPage();

			} else if ("s".equals(choNext) || "S".equals(choNext)) {
				System.out.println("\n--请输入商品关键字--");

				int gid = QueryPrint.querySettlement();// 当商品件数有且只有一件时返回商品gid号，商品已售空时返回
														// -1. >1件时返回-2 .
														// 查无此商品时返回-3
				...
				default:
					...
					ArrayList<Goods> goodsList = new QueryPrint()
							.queryGoodsKey(shoppingGid, null);
					if (goodsList == null || goodsList.size() == 0) {
						...
						do {
							...
							if (choicegoodsNum > gNum) {
								...
								double allPrice = Arith.mul(choicegoodsNum,
										gPrice);// 利用BigDecimal作乘法运算
								System.out.println("\t\t\t  购物车结算\n");
								System.out
										.println("\t\t商品名称\t商品单价\t购买数量\t总价\n");
								...
								do {
									...
									if ("y".equals(choShopping)
											|| "Y".equals(choShopping)) {
										System.out.println("\n总价：" + allPrice + " $");
										System.out.println("\n实际缴费金额");

										do {
											double amount = ScannerInfo();
											double balance = Arith.sub(amount,
													allPrice); // 用户交钱与购买物品总价间的差额
											if (balance < 0) {
